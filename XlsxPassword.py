# Подбор пароля для excel файлов
# необходимо загрузить библиотеку pywin32==300
...
def brute_excel_doc():
    ...
    try:
        password_length = input("Введите длину пароля, от - до символов, например 3 - 7: ")
    ...
    count = 0
    for pass_length in range(password_length[0], password_length[0] + 1):
        for password in itertools.product(possible_symbols, repeat=pass_length):
            ...
            try:
                opened_doc.Workbooks.Open(
                    r"d:\scripts\excel_password\test.xlsx",
                    False,
                    True,
                    None,
                    password
